...
BAD_IPS = ['116.255.235.199','208.81.75.97','220.250.64.225','92.242.140.21','132.216.45.219']
...
def graph_flows(g, df_conn):
    ...
    for con in df_conn.index:
        if df_conn.loc[con]["id.orig_h"] or df_conn.loc[con]["id.resp_h"] in BAD_IPS:
...
def graph_dns(g, df_dns):
    ...
    for i in df_dns.index:
        if df_dns.loc[i]["id.orig_h"] or df_dns.loc["id_resp_h"] in BAD_IPS:
...
def graph_http(g, df_http):
    ...
    for i in df_http.index:
        if df_http.loc[i]["id.orig_h"] or df_http.loc[i]["id.resp_h"] in BAD_IPS:
...
for filename in glob.glob(options.logdir+'/conn.*.log'):
    df_conn = readlog(filename[len(options.logdir)+1:], connection_ids)
    print "Number of events: %d" % len(df_conn.index)
    graph_flows(g, df_conn)

print "Graphing Files..."
for filename in glob.glob(options.logdir + '/files.*.log'):
    df_files = readlog(filename[len(options.logdir)+1:], connection_ids)
    print "Number of events: %d" % len(df_files.index)
    graph_files(g, df_files)

print "Graphing DNS Transactions..."
for filename in glob.glob(options.logdir + '/dns.*.log'):
    df_dns = readlog(filename[len(options.logdir)+1:], connection_ids)
    print "Number of events: %d" % len(df_dns.index)
    graph_dns(g, df_dns)

print "Graphing HTTP Transactions..."
for filename in glob.glob(options.logdir + '/http.*.log'):
    df_http = readlog(filename[len(options.logdir)+1:], connection_ids)
